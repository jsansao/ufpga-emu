...
- **Raspberry Pi 3 B+:** IP `192.168.1.127`, SSH `joao@`, key auth, aarch64, `/dev/gpiomem`, gpio group. Toolchain: `gcc`, `-lrt -lpthread`.
...
- Test suite: `pytest tests/` (75 parser) + `pytest tests/test_testbenches.py` (27 integração) = **102 coletados**; `test_stim` parametrizado via `STIM_CSV_MAP`/`_discover_stim_envs()` (19 CSVs em `examples/`).
...
- `firmware/src/main_esp8266.cpp` — ESP8266 main com 26 circuitos, single-core, UART Serial
- `firmware/src/main_due.cpp` — Due main com 26 circuitos, single-core loop
- `firmware/src/main_rpi.c` — Pi main com 26 circuitos, medição de freq
- `firmware/src/main_rp2040.cpp` — RP2040 firmware com USB CDC e multicore
...
- `firmware/src/main_pc.c` — lê `STIMULUS_CSV` (via `hal_stimulus_load`) antes do loop
