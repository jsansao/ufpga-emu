...
- ✅ **Editor de testbench-CSV na GUI (`stim-editor`):** intenção em `docs/intent/stim-editor.md`. A sim PC já aplicava os CSVs via `STIMULUS_CSV`, mas invisível — e 10 dos 19 CSVs usam o sinal `inputs`, que o runtime ignora silenciosamente. Agora a aba Exemplos tem área de texto editável pré-carregada: Simular valida (3 colunas, sinais contra o pinmap do `<nome>_main.c`, saídas rejeitadas) e roda com o texto (temporário, repo intocado); `inputs` packed é expandido no lado GUI, header sempre emitido (runtime descarta a linha 1). **120/120 testes** (+6 em `tests/test_gui.py`). Smoke Xvfb: `rst` estendido segura o contador em 0, adder_n soma 1+1 via `inputs`, erro didático p/ sinal inexistente.
- ✅ **Trace denso na sim GUI (`status` pumping):** o firmware imprime 1 snapshot/s, então a janela de 3 s rendia 2 linhas. Agora `run_example(..., poll_hz=5)` abre `stdin=PIPE` e bombeia `status\n` (reader thread acumula stdout) → ~18-20 snapshots por sim, sem mudar o toolchain. **122/122 testes** (+2 com fixture que compila counter 1×).
