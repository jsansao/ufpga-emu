    ...
    for (uint64_t t = 0; t <= MAX_TIME_US; t += HALF_PERIOD_US) {
        while (event_idx < num_events && events[event_idx].time_us <= t) {
            ...
            if (strcmp(e->signal, "rst") == 0)
                rst_val = e->value;
            else if (strcmp(e->signal, "run") == 0)
                run_val = e->value;
            else if (strncmp(e->signal, "instr[", 6) == 0) {
                int bit;
                if (sscanf(e->signal + 6, "%d]", &bit) == 1 && bit >= 0 && bit < 16) {
                    if (e->value) instr_val |= (1u << bit);
                    else instr_val &= ~(1u << bit);
                }
            }
            event_idx++;
        }
        ...
        uint32_t inputs = (clk << 0) | (rst_val << 1) | (run_val << 2)
                        | ((instr_val & 0xFFFF) << 3);
        ...
        if (t == 0)  check(t, "reset",     outputs, 0);
        if (t == 5)  check(t, "fetch_ldi", outputs, 1);
        if (t == 15) check(t, "exec_ldi",  outputs, 43521);
        if (t == 25) check(t, "fetch_out", outputs, 43522);
        if (t == 35) check(t, "exec_out",  outputs, 43522);
        if (t == 45) check(t, "fetch_hlt", outputs, 43523);
        if (t == 55) check(t, "exec_hlt",  outputs, 109059);
    }
