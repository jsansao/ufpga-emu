    ...
    for (uint64_t t = 0; t <= MAX_TIME_US; t += HALF_PERIOD_US) {
        while (event_idx < num_events && events[event_idx].time_us <= t) {
            ...
            if (strcmp(e->signal, "rst") == 0) {
                ...
                data_in_val = e->value;
            } else if (strncmp(e->signal, "parallel_load[", 14) == 0) {
                int bit;
                if (sscanf(e->signal + 14, "%d]", &bit) == 1 && bit >= 0 && bit < 8) {
                    if (e->value) parallel_load_val |= (1u << bit);
                    else parallel_load_val &= ~(1u << bit);
                }
            }
            event_idx++;
        }
        ...
        uint32_t inputs = (clk) | (rst_val << 1) | (load_val << 2) | (data_in_val << 3) | ((parallel_load_val & 0xFF) << 4);
