...
int main(void)
{
    ...
    for (uint64_t t = 0; t <= MAX_TIME_US; t += HALF_PERIOD_US) {
        ...
        uint32_t inputs = (clk << 0) | (rst_val << 1);

        uint32_t outputs;
        circuit_eval(&state, inputs, &outputs);
        steps++;

        uint8_t count = outputs & 0xFF;
        uint8_t overflow = (outputs >> 8) & 1;

        if (t == 0)   check(t, "reset",      count, 0);
        if (t == 5)   check(t, "count_1",    count, 1);
        if (t == 15)  check(t, "count_2",    count, 2);
        if (t == 35)  check(t, "count_4",    count, 4);
        if (t == 75)  check(t, "count_8",    count, 8);
        if (t == 155) check(t, "no_overflow", overflow, 0);
    }
