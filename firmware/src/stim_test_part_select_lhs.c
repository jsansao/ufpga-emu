...
int main(void)
{
    ...
    for (uint64_t t = 0; t <= MAX_TIME_US; t += HALF_PERIOD_US) {
        while (event_idx < num_events && events[event_idx].time_us <= t) {
            ...
            if (strncmp(e->signal, "value[", 6) == 0) {
                int bit;
                if (sscanf(e->signal + 6, "%d]", &bit) == 1 && bit >= 0 && bit < 4) {
                    if (e->value) value_val |= (1u << bit);
                    else value_val &= ~(1u << bit);
                }
        ...
        uint32_t inputs = (clk) | ((value_val & 0xF) << 1) | (load_val << 5);

        uint32_t outputs;
        circuit_eval(&state, inputs, &outputs);
        steps++;

        if (t == 0)   check(t, "init",   outputs, 0);
        if (t == 15)  check(t, "load8",  outputs, 128);
        if (t == 25)  check(t, "unload", outputs, 0);
    }
